...
def user_list():
    ...
    return jsonify("user_list.html", users=users)
...
#   return jsonify("user_profile.html", user=user, =user_ratings)
...
def register_form_post():
    ...
    if User.query.filter_by(userEmail=email).one():
        ...
        hash = hashlib.sha256(password.encode('utf-8')).hexdigest()
        new_user = User(userEmail=email, digest=hash)
    ...
    return jsonify({"success": False, "message": "Register Successful!"})
